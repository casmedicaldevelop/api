...
} satisfies Prisma.TvInsEventoInclude

@Injectable()
export class TvInsEventoService {
  constructor(private readonly prisma: PrismaService) {}

  async findAll(dto: ListTvInsEventoDto) {
    const page = dto.page ?? 1
    const limit = dto.limit ?? 20
    const skip = (page - 1) * limit

    const where: Prisma.TvInsEventoWhereInput = {}
    const and: Prisma.TvInsEventoWhereInput[] = []
  ...
  async findOne(id: number) {
    ...
    if (!record) throw new NotFoundException(`Registro tvins_evento con id ${id} no encontrado.`)
    return record
  }
  ...
  async create(dto: CreateTvInsEventoDto) {
  ...
  async update(id: number, dto: UpdateTvInsEventoDto) {
